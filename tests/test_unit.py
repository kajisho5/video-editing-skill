...
class FrameSemanticsAndEncodingTests(ExecutorHarness):
    ...
    def test_pinned_blocks_are_unchanged_since_0_1_0(self):
        ...
        `engine` is deliberately excluded: it is an additive block (docs/decisions.md ADR-008), free to drift as the
        ffmpeg-skill version range this Skill enforces changes (ADR-009 bumped it to >=0.10.0) — covered by
        test_drift_is_detected, not by this byte-identity freeze."""
        c = contract.skill_contract()
